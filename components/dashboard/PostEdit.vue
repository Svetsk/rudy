...
const { getArticleById, updatePost } = useArticleStore();

route.params.id && (currentPost.value = getArticleById(String(route.params.id)));

const state = reactive({
  title: currentPost.value?.title ?? "",
  author: currentPost.value?.author ?? "",
  imagePreview: currentPost.value?.imagePreview ?? "",
  subDescription: currentPost.value?.subDescription ?? "",
...
const submitForm = async () => {
  if (currentPost.value?.id) {
    ...
    if (resultName !== "Error" || resultName !== "Emty File") {
      ...
      const result = await updatePost(currentPost.value?.id, state);
      console.log(result);
